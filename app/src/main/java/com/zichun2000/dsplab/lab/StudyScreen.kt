...
fun StudyScreen() {
    ...
    Column(Modifier.padding(horizontal = 16.dp, vertical = 12.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(5.dp)) {
            Text(if (stage == 0) "1 · Pre-test" else if (stage == 1) "2 · Learning activity" else "3 · Post-test", style = MaterialTheme.typography.titleMedium)
            Text(when (stage) { 0 -> "Check your starting DSP knowledge."; 1 -> "Complete Labs 01–05, then take the post-test."; else -> "Check what you learned after the laboratory activities." })
        }}
        if (stage == 1) {
            Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(5.dp)) {
            ...
            items.forEachIndexed { number, item ->
                Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(2.dp)) {
                    ...
                    item.options.forEachIndexed { index, option -> Row(Modifier.fillMaxWidth()) {
            ...
            Button(onClick = {
                if (stage == 0) { preScore = score; store.save(ExperimentRecord("PRE_TEST", emptyMap(), "score=$score/${items.size}")); answers.clear(); stage = 1 }
                else { postScore = score; store.save(ExperimentRecord("POST_TEST", emptyMap(), "score=$score/${items.size}")) }
            }, modifier = Modifier.fillMaxWidth()) { Text(if (stage == 0) "Submit Pre-test" else "Submit Post-test") }
